//===-- Y86InstrBuilder.h - Functions to aid building Y86 insts -*- C++ -*-===//
//
...
// MachineInstrBuilder.h file to handle Y86'isms in a clean way.
//
// The BuildMem function may be used with the BuildMI function to add entire
// memory references in a single, typed, function call.  Y86 memory references
...
#ifndef LLVM_LIB_TARGET_Y86_Y86INSTRBUILDER_H
#define LLVM_LIB_TARGET_Y86_Y86INSTRBUILDER_H
...
/// Y86AddressMode - This struct holds a generalized full Y86 address mode.
/// The base register can be a frame index, which will eventually be replaced
/// with BP or SP and Disp being offsetted accordingly.  The displacement may
/// also include the offset of a global value.
struct Y86AddressMode {
  enum {
    RegBase,
    FrameIndexBase
  } BaseType;
  ...
  void getFullAddress(SmallVectorImpl<MachineOperand> &MO) {
    ...
    if (BaseType == Y86AddressMode::RegBase)
      MO.push_back(MachineOperand::CreateReg(Base.Reg, false, false, false,
                                             false, false, false, 0, false));
    else {
      assert(BaseType == Y86AddressMode::FrameIndexBase);
...
/// given operand.
static inline Y86AddressMode getAddressFromInstr(const MachineInstr *MI,
                                                 unsigned Operand) {
  Y86AddressMode AM;
  const MachineOperand &Op0 = MI->getOperand(Operand);
  if (Op0.isReg()) {
    AM.BaseType = Y86AddressMode::RegBase;
    AM.Base.Reg = Op0.getReg();
  } else {
    AM.BaseType = Y86AddressMode::FrameIndexBase;
...
addFullAddress(const MachineInstrBuilder &MIB,
               const Y86AddressMode &AM) {
  assert(AM.Scale == 1 || AM.Scale == 2 || AM.Scale == 4 || AM.Scale == 8);

  if (AM.BaseType == Y86AddressMode::RegBase)
    MIB.addReg(AM.Base.Reg);
  else {
    assert(AM.BaseType == Y86AddressMode::FrameIndexBase);
...
/// either machine code emission or assembly output. In PIC mode on Y86-32,
...
#endif // LLVM_LIB_TARGET_Y86_Y86INSTRBUILDER_H
