...
void Y86DAGToDAGISel::selectFrameIndex(SDNode *SN, SDNode *N, unsigned Offset) {
  ...
  if (SN->hasOneUse())
    ...
    ReplaceNode(SN, CurDAG->getMachineNode(Opc, DL, VT, TFI, OffsetConstant));
}
...
bool Y86DAGToDAGISel::selectAddr(SDNode *Parent, SDValue N, SDValue &Base,
  ...
  Scale = Zero;
  Index = CurDAG->getRegister(Y86::NoRegister, VT);
  Disp = Zero;
  Segment = Zero;
  LLVM_DEBUG(dbgs() << "start selectAddr"; N.dump(); Parent->dump());
